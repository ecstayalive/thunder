...
class JaxExecutor:
    ...
    def optimize(
        ...
        if objs_key not in self._compiled_step_cache:
            self._compiled_step_cache[objs_key] = jax.jit(
                ...
                static_argnames=["models", "max_grad_norm", "target"],
                donate_argnums=(0, 1) if self.donate else None,
            )
        jit_fn = self._compiled_step_cache[objs_key]
        metrics, new_params, new_opt_state = jit_fn(
            params_tree, opt_state, ctx.batch, ctx.models, ctx.params, target, max_grad_norm
        )
    ...
    def _jit_update(
        ...
        def loss_fn(p):
            current_params = {**params, target: p}
            total_loss = 0.0
            metrics_acc = {}

            for obj in objectives:
                l, m = obj.forward(batch, models, current_params)
                total_loss += l
                metrics_acc.update(m)
            return total_loss, metrics_acc

        (loss, metrics), grads = jax.value_and_grad(loss_fn, has_aux=True)(target_params)
        if max_grad_norm > 0:
            grad_norm = optax.global_norm(grads)
            scale = jnp.clip(max_grad_norm / (grad_norm + 1e-6), max=1.0)
            grads = jax.tree_util.tree_map(lambda g: g * scale, grads)
            metrics["grad_norm"] = grad_norm
        updates, new_opt_state = tx.update(grads, opt_state, target_params)
        new_params = optax.apply_updates(target_params, updates)
    ...
    def init(
        ...
        for opt_key, cfg in optim_config.items():
            ...
            if target_name not in params_map:
                raise ValueError(f"Optimizer target '{target_name}' not found in models.")
            target_params = params_map[target_name]
            ...
            opt_states[opt_key] = tx.init(target_params)
