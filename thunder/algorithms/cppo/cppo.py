    ...
    def __init__(
        ...
        **kwargs,
        ...
        if init_std is not None:
            print("Warning: `init_std` for PPO is deprecated!")
            self.actor.set_exploration_std(init_std)
        ...
        # storage
        self.storage = None
        self.transition: Optional[RolloutBuffer.Transition] = None
        ...
        self.torso_smooth_weight = 1 / torch.tensor([0.3, 0.6, 0.6] * 4, device=self.device)
        self.arm_smooth_weight = 1 / torch.tensor(
            [0.6, 1.0, 1.0, 1.0, 0.6, 0.6], device=self.device
        )
        self.smooth_weight = torch.cat((self.torso_smooth_weight, self.arm_smooth_weight))

    def init_storage(self):
        self.storage = RolloutBuffer(self.num_collects, self.device)
        self.transition = self.storage.Transition()

    @staticmethod
    def orthogonal_init(modules, scale=np.sqrt(2), rescale=False):
        for module in modules:
            if isinstance(module, torch.nn.Linear):
                torch.nn.init.orthogonal_(module.weight, gain=scale)
                # torch.nn.init.zeros_(m.bias)
            elif isinstance(module, torch.nn.LSTM):
                torch.nn.init.orthogonal_(module.weight_hh_l0, gain=scale)
                torch.nn.init.orthogonal_(module.weight_ih_l0, gain=scale)
                torch.nn.init.zeros_(module.bias_hh_l0)
                torch.nn.init.zeros_(module.bias_ih_l0)
        if rescale:
            module.weight.data *= 0.01

    @torch.inference_mode()
    def act(self, actor_obs, critic_obs=None) -> np.ndarray:
        t = self.transition
