...
class RolloutBuffer(Buffer):
    ...
    def _recurrent_sampler(
        self, num_mini_batches, num_repetitions
    ) -> Generator[Batch, None, None]:
    ...
    @staticmethod
    def slice_hidden_features(
        hidden_features: list[torch.Tensor], indices: slice, mask: torch.Tensor
        ...
        Args:
            hidden_features: A list of tensor representing the hidden state of the
                recurrent neural network.
                :shape: `[time_steps, num_layers, num_envs, hidden_dim]`
            indices: A slice object specifies which elements of the `obs` tensor should
                be selected. It is used to slice the `obs` tensor along the second dimension
        ...
        hidden_batch = tuple(map(slice_hidden_fn, hidden_features))
        if len(hidden_batch) == 1:
            hidden_batch = hidden_batch[0]
        return hidden_batch
